...
contract SponsorPaymaster is IPaymaster, Ownable, EIP712 {
    ...
    uint256 constant MARKUP_DENOMINATOR = 1e4;
    // The nominator used for calculating the sponsorship ratio
    uint256 constant SPONSORSHIP_RATIO_DENOMINATOR = 1e4;
    ...
    constructor(address _verifier) EIP712("SponsorPaymaster", version){
        ...
        returns (bytes4 magic, bytes memory context)
    {
        ...
        ) = abi.decode(data, (uint64, uint256, address, bytes));
        ...
        if (protocolAddress != address(0)) {
            ...
            requiredETHProtocol =
                (requiredETH * getMarkup(protocolAddress) ) /
                MARKUP_DENOMINATOR;
        ...
        emit SponsoredPaymasterTransaction(protocolAddress, userAddress, requiredETH);
    ...
    function postTransaction(
        ...
        (
            ...
            uint256 requiredETHProtocol,
            uint256 requiredETH
        ) = abi.decode(_context, ( address, uint256, uint256));
        ...
        if (requiredETHProtocol > 0) {
            ...
            uint256 refundEthProtocol= (requiredETH *
    ...
    function _isValidSignature(
        ...
        (address recoveredAddress, ECDSA.RecoverError error2) = ECDSA
            .tryRecover(ethSignedMessageHash, signature);
