...
contract SponsorshipVaultV2 is Ownable2Step{
    // Used to identify the contract version
    string public constant version = "2.0";

    // Mapping from address to amount of ETH
    mapping(address protocol => uint256 amount) public balances;

    // The paymaster address that can withdraw from any account
    mapping(address paymaster => bool) public isPaymaster;
    ...
    // Event for new paymaster added
    event PaymasterAdded(address indexed paymaster);
    // Event for removal of paymaster
    ...
    modifier onlyPaymaster() {
        if (!isPaymaster[msg.sender]) revert Errors.NotFromPaymaster();
        _;
    }
    ...
    function addPaymaster(address _newPaymaster) public onlyOwner{
        if (_newPaymaster == address(0)) revert Errors.InvalidAddress();
        if(isPaymaster[_newPaymaster]) revert Errors.InvalidAddress();
        isPaymaster[_newPaymaster] = true;
        emit PaymasterAdded(_newPaymaster);
    }

    function removePaymaster(address _paymaster) public onlyOwner{
        if(_paymaster == address(0)) revert Errors.InvalidAddress();
        if(!isPaymaster[_paymaster]) revert Errors.InvalidAddress();
