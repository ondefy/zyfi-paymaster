...
const Verifier_PK = "0x8f6e509395c13960f501bc7083450ffd0948bc94103433d5843e5060a91756da"
...
describe("ERC20Paymaster", function () {
	...
	before(async function () {
		...
		paymaster = await hre.zkUpgrades.deployProxy(deployer.zkWallet, contract, [verifier.address], {
	...
	async function executeTransaction(
		...
		const expiration = BigNumber.from((await helper.getTimestamp()).add(TX_EXPIRATION))
	...
	it("Should validate and pay for paymaster transaction", async function () {
		...
		expect(await erc20.allowance(user.address, paymaster.address)).to.be.eql(BigNumber.from(0))
	})
	...
	it("should revert if unsupported paymaster flow", async function () {
		await expect(executeTransaction(user, erc20.address, "General")).to.be.rejectedWith("Unsupported paymaster flow")
	})
